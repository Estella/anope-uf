...
class CharybdisProto : public IRCDProto
{
 public:
	...
	CharybdisProto(Module *creator) : IRCDProto(creator, "ircd-chatd 0.2.2+")
	{
		...
		CanSZLine = true;
		CanSVSNick = true;
		CanSVSJoin = true; // if m_force loaded
		CanSVSHold = true;
		CanSetVHost = true;
		RequiresID = true;
		MaxModes = 4;
	...
	{
		if (chan.empty() || (chan[0] != '#' && chan[0] != '"' /*"*/ && chan[0] != '1' && chan[0] != '2' && chan[0] != '3' && chan[0] != '4'
		    && chan[0] != '5' && chan[0] != '6' && chan[0] != '7' && chan[0] != '8' && chan[0] != '9' && chan[0] != '0')
		    || chan.length() > Config->GetBlock("networkinfo")->Get<unsigned>("chanlen"))
			return false;

		if (chan.length() == 9 && (chan[0] == '1' && chan[0] == '2' && chan[0] == '3' && chan[0] == '4' && chan[0] == '5'
		    && chan[0] == '6' && chan[0] == '7' && chan[0] == '8' && chan[0] == '9' && chan[0] == '0'))
			return false;

		for (int i=0; i<chan.length(); i++) {
			if (i == 0 && (chan[0] == '#' || chan[0] == '"')) break;
			if (chan[i] != '1' && chan[i] != '2' && chan[i] != '3' && chan[i] != '4'
			    && chan[i] != '5' && chan[i] != '6' && chan[i] != '7' && chan[i] != '8' && chan[i] != '9' && chan[i] != '0') return false;
		}
	...
	bool IsNickValid (const Anope::string &nick) anope_override
	{
		for (int i=0; i<nick.length(); i++)
			if ((unsigned char)nick[i] < 45 || (unsigned char)nick[i] == 46 || ((unsigned char)nick[i] >= 58 && (unsigned char)nick[i] <= 64) ||
			    ((unsigned char)nick[i] >= 126 && (unsigned char)nick[i] <= 127) || (unsigned char)nick[i] > 245)
				return false;
		return true;
	}
	...
	{
		UplinkSocket::Message(Me) << "ENCAP * SVSJOIN " << user->GetUID() << " " << chan;
	}
...
class ProtoCharybdis : public Module
{
	...
	void AddModes()
	{
		...
		ModeManager::AddUserMode(new UserModeOperOnly("GOD", 'p'));
		ModeManager::AddUserMode(new UserModeNoone("SSL", 'Z'));
		ModeManager::AddUserMode(new UserModeNoone("SCTP", 'A'));
		ModeManager::AddUserMode(new UserMode("HIDECFP", 'F'));
		ModeManager::AddUserMode(new UserMode("CLOAK", 'x'));

		/* IRCd-chatd is a snowflake of an IRCd */
		ModeManager::AddChannelMode(new ChannelModeStatus("VOICE", 'v', '+', 0));
		ModeManager::AddChannelMode(new ChannelModeStatus("HALFOP", 'h', '%', 1));
		ModeManager::AddChannelMode(new ChannelModeStatus("OP", 'o', '@', 2));
		ModeManager::AddChannelMode(new ChannelModeStatus("PROTECT", 'a', '&', 3));
		ModeManager::AddChannelMode(new ChannelModeStatus("OWNER", 'w', '~', 4));
		ModeManager::AddChannelMode(new ChannelModeStatus("OWNERBOT", 'W', '!', 4));

		/* b/e/I */
		ModeManager::AddChannelMode(new ChannelModeList("QUIET", 'q'));

		/* Add channel modes */
		ModeManager::AddChannelMode(new ChannelMode("ANONYMISE", 'A'));
		ModeManager::AddChannelMode(new ChannelMode("BLOCKCOLOR", 'c'));
		ModeManager::AddChannelMode(new ChannelMode("AUDITORIUM", 'D'));
 ...
 public:
	ProtoCharybdis(const Anope::string &modname, const Anope::string &creator) : Module(modname, creator, PROTOCOL | VENDOR),
		...
		message_bmask("IRCDMessage", "ircd-chatd/bmask", "ratbox/bmask"),
		message_join("IRCDMessage", "ircd-chatd/join", "ratbox/join"),
		message_nick("IRCDMessage", "ircd-chatd/nick", "ratbox/nick"),
		message_pong("IRCDMessage", "ircd-chatd/pong", "ratbox/pong"),
		message_sid("IRCDMessage", "ircd-chatd/sid", "ratbox/sid"),
		message_sjoin("IRCDMessage", "ircd-chatd/sjoin", "ratbox/sjoin"),
		message_tb("IRCDMessage", "ircd-chatd/tb", "ratbox/tb"),
		message_tmode("IRCDMessage", "ircd-chatd/tmode", "ratbox/tmode"),
		message_uid("IRCDMessage", "ircd-chatd/uid", "ratbox/uid"),
